    ...
    <div className="min-h-screen overflow-hidden bg-background text-foreground selection:bg-gold/30">
      ...
      <div className="relative mx-auto max-w-[1440px] px-4 py-5 sm:px-6 lg:px-10">
        ...
        <main>
          ...
          {screen === "execution" && <Execution name={executionName} batch={batch} kris={kris.filter((k) => k.active)} onBack={() => go("dashboard")} />}
        </main>
...
function Dashboard({ kris, query, risk, setQuery, setRisk, onBack, onAdd, onExecute, onToggle, onDelete }: { kris: Kri[]; query: string; risk: string; setQuery: (v: string) => void; setRisk: (v: string) => void; onBack: () => void; onAdd: () => void; onExecute: (name?: string) => void; onToggle: (id: string) => void; onDelete: (id: string) => void }) {
  return <section className="screen-enter mt-10"><PageTitle eyebrow="Source to Pay" title="S2P · KRI Management" onBack={onBack} aside={<span className="flex items-center gap-2 text-xs text-muted-foreground"><span className="status-pulse size-1.5 rounded-full bg-warning" />2 thresholds breached</span>} />
    ...
    <div className="mt-4 overflow-x-auto rounded-2xl border border-border bg-card/40"><table className="w-full min-w-[900px] text-left"><thead><tr className="border-b border-border bg-surface/60 text-[10px] uppercase tracking-[0.18em] text-muted-foreground"><th className="px-5 py-3 font-medium">Status</th><th className="px-5 py-3 font-medium">KRI ID</th><th className="px-5 py-3 font-medium">Name</th><th className="px-5 py-3 font-medium">Category</th><th className="px-5 py-3 font-medium">Risk threshold</th><th className="px-5 py-3 font-medium">Last executed</th><th className="px-5 py-3 text-right font-medium">Actions</th></tr></thead><tbody>{kris.map((kri) => <tr key={kri.id} className={`border-b border-border/50 transition-colors last:border-0 hover:bg-surface-raised/40 ${kri.active ? "" : "opacity-50"}`}><td className="px-5 py-4"><button type="button" role="switch" aria-checked={kri.active} aria-label={`${kri.active ? "Deactivate" : "Activate"} ${kri.name}`} onClick={() => onToggle(kri.id)} className={`relative h-5 w-9 shrink-0 rounded-full border transition-colors ${kri.active ? "border-gold/60 bg-gold" : "border-border bg-surface"}`}><span className={`absolute top-0.5 size-3.5 rounded-full transition-all ${kri.active ? "left-[18px] bg-primary-foreground" : "left-0.5 bg-muted-foreground"}`} /></button></td><td className="px-5 py-4 font-mono text-xs text-gold-bright">{kri.id}</td><td className="px-5 py-4 text-sm font-medium">{kri.name}{!kri.active && <span className="ml-2 rounded-full border border-border px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wider text-muted-foreground">Inactive</span>}</td><td className="px-5 py-4 text-xs text-muted-foreground">{kri.category}</td><td className={`px-5 py-4 text-xs ${kri.risk === "High" ? "text-risk" : kri.risk === "Medium" ? "text-warning" : "text-success"}`}>{kri.threshold}</td><td className="px-5 py-4 text-xs text-muted-foreground">{kri.executed}</td><td className="px-5 py-4"><div className="flex justify-end gap-2"><Button variant="premium" size="compact" disabled={!kri.active} title={kri.active ? "Execute this KRI" : "Activate this KRI to execute it"} onClick={() => onExecute(kri.name)}><Play />Execute</Button><Button variant="console" size="compact" onClick={() => toast.info(kri.name, { description: `${kri.category} · ${kri.threshold} · Last executed ${kri.executed}` })}>View Details</Button><Button variant="console" size="compact" aria-label={`Delete ${kri.name}`} title={`Delete ${kri.name}`} className="text-risk hover:border-risk/50" onClick={() => onDelete(kri.id)}><Trash2 /></Button></div></td></tr>)}</tbody></table>{kris.length === 0 && <div className="p-12 text-center text-sm text-muted-foreground">No KRIs match your filters.</div>}</div>
  </section>;
}
...
function Intake({ onCancel, onSave }: { onCancel: () => void; onSave: (kri: Kri) => void }) {
  ...
  const submit = (e: FormEvent) => { e.preventDefault(); if (step < 3) { setStep(step + 1); return; } if (!name.trim()) { toast.error("Enter a KRI name"); setStep(1); return; } onSave({ id: `S2P-${String(50 + Math.floor(Math.random() * 40)).padStart(3, "0")}`, name, category: source, risk: category, threshold: `${min}–${max}`, executed: "Not run", active: true }); };
...
function Execution({ name, batch, kris, onBack }: { name: string; batch: boolean; kris: Kri[]; onBack: () => void }) {
  const [run, setRun] = useState(0); const [step, setStep] = useState(0); const [tick, setTick] = useState(0);
  ...
  useEffect(() => { if (!batch) return; setTick(0); const total = kris.length * 3 + 2; const interval = window.setInterval(() => setTick((v) => { if (v >= total) { window.clearInterval(interval); return v; } return v + 1; }), 700); return () => window.clearInterval(interval); }, [run, batch, kris.length]);
  const batchDone = batch && tick >= kris.length * 3 + 2;
  return <section className="screen-enter mt-10"><div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-gold/30 bg-gradient-to-r from-surface-raised to-surface p-5"><PageTitle compact eyebrow={batch ? "Batch Processing" : "Live Processing"} title={`S2P Execution Engine · ${name}`} onBack={onBack} /><Button variant="console" onClick={() =>
